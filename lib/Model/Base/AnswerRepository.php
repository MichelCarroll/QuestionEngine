...
/**
 * Base class of repository of Model\Answer document.
 */
abstract class AnswerRepository extends \Mandango\Repository
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        $this->documentClass = 'Model\Answer';
        $this->isFile = false;
        $this->collectionName = 'model_answer';
    ...
    public function fixMissingReferences($documentsPerBatch = 1000)
    {
        $skip = 0;
        do {
            $cursor = $this->getCollection()->find(array('question' => array('$exists' => 1)), array('question' => 1))->limit($documentsPerBatch)->skip($skip);
            $ids = array_unique(array_values(array_map(function ($result) { return $result['question']; }, iterator_to_array($cursor))));
            if (count($ids)) {
                $collection = $this->getMandango()->getRepository('Model\Question')->getCollection();
                ...
                if ($idsDiff = array_diff($ids, $referenceIds)) {
                    $this->remove(array('question' => array('$in' => $idsDiff)), array('multiple' => 1));
                }
            }
